...
A hipótese mostrou-se infundada, pois não existe uma tendência de diferença na litragem dos motores entre os anos. Para sumarizar essas medidas, pode-se fazer
...
```
Constata-se que na verdade carros mais recentes têm motores uma média maior de litragem, entretanto tem uma média menor de consumo, o que pode ser explicado pelo avanço da engenharia de motores, que trouxe modelos mais eficientes.

```{r}
print(summary(mpg_1999$hwy))
print(summary(mpg_2008$hwy))
```

É valida investigar se determinada marca foca sua produção em carros de determinada categoria. Isso pode ser alcançado ao criar o gráfico

```{r}
ggplot(mpg, aes(manufacturer, class, colour = manufacturer)) + 
  geom_count()
```
